...
def equal_weight(vec1, vec2):
    ...
    numerator = sum([1.0 - abs(float(vec1[key]-vec2[key]) / (vec1[key]+vec2[key])) for key in intersection])
...
def get_file_content(filename):
    ...
    if "http://" in filename:
        try:
            return urllib.request.urlopen(filename).read()
        except urllib.error.URLError as exp:
            print("Error: cannot open %s" % filename, file=sys.stderr)
            raise exp
    else:
        try:
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: % python <FILENAME1> <FILENAME2>", file=sys.stderr)
    else:
